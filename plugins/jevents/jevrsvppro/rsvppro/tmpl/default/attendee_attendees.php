...
if (count($this->attendees) > 0)
{
	...
	if ($this->attendees && count($this->attendees) > 0)
	{
		...
		foreach ($this->attendees as $attendee)
		{
			...
			if (!$attendee->name)
			{
				if ($user->id==0 || !($user->id == $this->row->created_by() || JEVHelper::isAdminUser($user))){
	...
	if ($this->params->get("sortableattendees", 0)  && (JRequest::getCmd("jevtask")=="icalrepeat.detail" || JRequest::getCmd("jevtask")=="icalevent.detail"))
	{
		...
		if (!$registry->get("calledthis",0)) {
			$doc = JFactory::getDocument();
			$doc->addScriptDeclaration("window.addEvent('domready', function(){if (document.getElement('.jevattendeestable')) {document.getElement('.jevattendeestable').style.height=($('jevattendees').getSize().y+50)+'px';}});");
			$registry->set("calledthis",1);
		}
	}
	if ($this->jomsocial)
