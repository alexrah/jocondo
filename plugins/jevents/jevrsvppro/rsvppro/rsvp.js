...
function showSubmitButton(){
	...
	$("rsvppro_admintable").style.display="table";
	...
	buildElements: function() {
		...
		this.mainDiv = new Element('div', {
			'styles' : {
				...
				'padding': 0,
				'display':'none',
				'left': this.el.getPosition(this.el.getParent()).x-9*(this.el.getSize().x|30),
		...
		var divWidth = Math.min(itemCount,24) * 26;
	...
	itemOver: function(el) {
		...
		if(this.panelVisible) {
			...
			if(this['panel' + (level + 1)]) {
				this.movePanel(level + 1,el.xvalue);
                        }
		}
	},

	movePanel: function(level,xvalue) {
		...
		//this['panel' + level].setStyle('left', currentItem.getCoordinates([this.mainDiv]).left);
                if (xvalue<13){
                   this['panel' + level].setStyle('right',"auto");
		this['panel' + level].setStyle('left',currentItem.offsetLeft+currentItem.offsetParent.offsetLeft );
                }
                else{
                 this['panel' + level].setStyle('left',"auto");
		this['panel' + level].setStyle('right',0 );
                }
...
function priceJevrList(name, prices){
	...
	elems.each(function(elem){
		...
		if (name.replace("xmlfile_", "")!=name){
			indivcount = elem.getParent().id.replace("rsvp_"+name.replace("xmlfile_", "")+"_span_","");
		}
...
function priceJevrRadio(name, prices){
	...
	elems.each(function(elem){
		...
		if (indivcount == "" && elem.getParent().tagName.toLowerCase()=="label") {
			...
			if (name.replace("xmlfile_", "")!=name){
				indivcount =  elem.getParent().getParent().id.replace("rsvp_"+name.replace("xmlfile_", "")+"_span_","");
			}
		} 
		// NB this is the name not the element!
		if (JevrConditionalFields.isVisible(name, indivcount)){
			...
			if (rbox.checked) {
				...
				if (rbox.value.toUpperCase()=="NONE" ){					
					...
					if (allrepeats ) {
						...
						//if (document.adminForm.evid.value==0) $('custom_rsvp_allrepeats0').checked = true;
					}
					if (allinvites ) {
						allinvites.style.display="block";
						// uncomment if you want new events to be tracked by each specific repeat by default
						//if (document.adminForm.evid.value==0) $('custom_rsvp_allinvites0').checked = true;
					}
					if (allreminders ) {
						allreminders.style.display="block";
						// uncomment if you want new events to be tracked by each specific repeat by default
						//if (document.adminForm.evid.value==0) $('custom_rsvp_remindallrepeats0').checked = true;
					}
				}
			}
...
var JevrFees = {
	...
	calculate:function (form){
		if ($('jevrtotalfee') && $('guestcount')){
			...
			if ($('jevrdeposit') && $('xmlfile_totalfee')) {
				$('jevrdeposit').innerHTML = " ( "+rsvpMoneyFormat($('xmlfile_totalfee').value * parseFloat($('jevrdepositpercentage').innerHTML)/100)+" ) " ;
			}
		}
	}
}
...
function moveEmailFieldDown() {
	if ($('jevattend_email')){
		...
		var elem = $('jevattend_email').getParent().getParent();
		var target = $('rsvppro_admintable').getElements('tr');
		if (target){
