...
class JFormFieldJevrflatfee extends JevrField
{
	...
	private function overridePrice()
	{
		...
		if (!isset($this->overridenPrice)) {
			$this->overridenPrice = false;
		}

		if (!$this->overridenPrice  && isset($this->rsvpdata) && isset($this->rsvpdata->overrideprice) && trim($this->rsvpdata->overrideprice)!==""){
			$this->value = floatval($this->rsvpdata->overrideprice);
			$this->addAttribute("amount",  $this->value);
			$this->addAttribute("default",  $this->value);
			$this->overridenPrice  = true;
		}
