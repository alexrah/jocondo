	...
	public function paymentForm(&$attendee)
	{
		...
		foreach ($fieldgroup as & $field)
		{
			...
			if ( JRequest::getString("tmpl","")=="component"){
				...
				$html = '
<form action="' . JRoute::_('index.php?option=com_rsvppro&task=accounts.paymentpage&gateway=' . $gateway . '&Itemid=' . $Itemid . $tmpl) . '" method="POST" name="jevrpaymentform">
	...
	public function getTicket($attendee, $rsvpdata, $event)
	{
		...
		if (count($matches) == 2)
		{
			...
			$tickettemplate = $matches[1]."<hr/><br/>";
