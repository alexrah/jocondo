...
class RsvpInviteeHelper
{
	...
	private function parseMessage($message, $rsvpdata, & $row, $name, $requirelogin = false, $currentinvitee, $parsetype = "subject")
	{
		...
		if ($this->params->get("invite_icals", 0) && $parsetype == "message")
		{
			...
			if (!empty($icalEvents))
			{
				...
				foreach ($icalEvents as $a)
				{
					...
					$html .= $this->setDescription(strip_tags($a->content())) . "\r\n";
